...
class Qliro_One_Request_Order {
	...
	public function get_order_line_items( $order_item ) {
		...
		return array(
			...
			'unit_price'        => number_format( ( $order_item->get_total() / $order_item->get_quantity() ), 2 ),
			'unit_price_no_tax' => number_format( ( ( $order_item->get_total() - $order_item->get_total_tax() ) / $order_item->get_quantity() ), 2 ),
		);
	}
	...
	public function get_item_unit_price( $order, $order_item = false ) {
		if ( $this->separate_sales_tax ) {
			...
			$item_subtotal = ( ( number_format( $order_item->get_total(), wc_get_price_decimals(), '.', '' ) +
			                     number_format( $order_item->get_total_tax(), wc_get_price_decimals(), '.', '' ) ) / $order_item->get_quantity() ) * 100;
		}
