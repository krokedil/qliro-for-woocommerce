...
class Qliro_One_Helper_Cart {
	...
	public static function get_shipping_tax_rate( $shipping_rate ) {
		$rate = 0.0 !== (float) $shipping_rate->get_cost() ? array_sum( $shipping_rate->get_taxes() ) / $shipping_rate->get_cost() * 100 : 0;
	...
	private static function get_fee_tax_rate( $fee ) {
		return self::format_vat_rate( 0.0 !== (float) $fee->amount ? ( $fee->tax / $fee->amount ) * 100 : 0 );
	}
